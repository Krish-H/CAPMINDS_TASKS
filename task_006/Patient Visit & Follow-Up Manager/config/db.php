...
define('APP_KEY', 'v8y/B?E(G+KbPeShVmYq3t6w9z$C&F)H'); // 32 chars for AES-256
define('APP_IV', '1234567890123456'); // 16 chars

function encode_id($id) {
    if (!$id) return '';
    $encrypted = openssl_encrypt((string)$id, 'AES-256-CBC', APP_KEY, 0, APP_IV);
    return str_replace(['+', '/', '='], ['-', '_', ''], base64_encode($encrypted));
}

function decode_id($hash) {
    if (!$hash) return null;
    $b64 = base64_decode(str_replace(['-', '_'], ['+', '/'], $hash));
    $decrypted = openssl_decrypt($b64, 'AES-256-CBC', APP_KEY, 0, APP_IV);

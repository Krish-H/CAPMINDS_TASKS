...
switch ($method) {
    ...
    case 'POST':
        ...
        if ($checkStmt->get_result()->num_rows > 0) {
            sendResponse(400, 'This date and time slot is already taken.');
        }
    ...
    case 'PUT':
        if (isset($input['status_only']) && $input['status_only'] == true) {
            ...
            $checkStmt = $conn->prepare("SELECT id FROM appointments WHERE appointment_date = ? AND appointment_time = ? AND id != ?");
            $checkStmt->bind_param("ssi", $date, $time, $id);
            $checkStmt->execute();
            if ($checkStmt->get_result()->num_rows > 0) {
                sendResponse(400, 'Double booking error: This date and time slot is already taken.');
            }
